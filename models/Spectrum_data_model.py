...
class Spectrum_data(Base):
    ...
    precursor_mz:Mapped[float] = mapped_column(nullable=True, index=True)
    exact_mass:Mapped[float] = mapped_column(index=True, nullable=True)
    collision_energy:Mapped[str] = mapped_column(String(100),default="0")
    ...
    ion_mode:Mapped[str] = mapped_column(String(45),nullable=True, index=True)
    ##ms2_spectrum:Mapped[Optional[str]] = mapped_column(String(5000),nullable=True)
    ms2_spectrum:Mapped[LONGTEXT] = mapped_column()
